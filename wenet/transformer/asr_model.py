...
class ASRModel(torch.nn.Module):
    ...
    def _calc_att_loss(
        ...
        # print(f'target in : {ys_in_pad}')
        # print(f'target in size : {ys_in_pad.size()}')
        # print(f'target out : {ys_out_pad}')
        # print(f'target out size : {ys_out_pad.size()}')
        ys_in_lens = ys_pad_lens + 1
        # print(f'input length : {ys_in_lens}')

        # reverse the seq, used for right to left decoder
        r_ys_pad = reverse_pad_list(ys_pad, ys_pad_lens, float(self.ignore_id))
        r_ys_in_pad, r_ys_out_pad = add_sos_eos(r_ys_pad, self.sos, self.eos,
                                                self.ignore_id)
        # print(f'right target in : {r_ys_in_pad}')
        # print(f'right target in size : {ys_in_pad.size()}')
        # print(f'right target out : {r_ys_out_pad}')
        # print(f'right target out size : {r_ys_out_pad.size()}')
        # print(f'encoder mask : {encoder_mask}')
        # print(f'encoder mask size : {encoder_mask.size()}')
        # exit(1)
        # 1. Forward decoder
        ...
        # import os
        # print(os.path.realpath(__file__))
        # print(f'decoder out shape : {decoder_out.size()}')
        # print(f'right decoder out shape : {r_decoder_out.size()}')
        # exit(1)
