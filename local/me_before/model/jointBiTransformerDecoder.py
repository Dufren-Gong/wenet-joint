...
class JointTransformerDecoder(torch.nn.Module):
    ...
    def forward(
        ...
        x, _ = self.embed(tgt)
        x = self._add_dysf_embedding_to_target(x, dysf_in_pad)
    ...
    def _add_dysf_embedding_to_target(self, x: torch.Tensor, dysf_label: torch.Tensor) -> torch.Tensor:
        feature_length = x.size(-1)
        dysf_embedding = torch.unsqueeze(dysf_label, dim=-1)
        dysf_embedding = dysf_embedding.repeat(1, 1, feature_length).to(x.device)
        x = torch.add(x, dysf_embedding)
        return x
    ...
    def forward_layers(self, x: torch.Tensor, tgt_mask: torch.Tensor,
        ...
        for layer in self.decoders:
            #cross attention golden result and ctc predict info
    ...
    def forward_one_step(
        ...
        if self.normalize_before:
            #第二个维度为maxlen_out,意味取出最后一个chunk的log prob
...
class JointBiTransformerDecoder(torch.nn.Module):
    """Base class of Transfomer decoder module.
    Args:
        vocab_size: output dim
        encoder_output_size: dimension of attention
    ...
    def __init__(
        ...
        super().__init__()
        #设定asr output layer
        self.asr_output_layer = torch.nn.Linear(encoder_output_size, vocab_size)
        self.r_asr_output_layer = torch.nn.Linear(encoder_output_size, vocab_size)
        self.use_sdpa = use_sdpa
        self.tie_word_embedding = tie_word_embedding
        #Decoder layer不直接经过linner层输出token分类结果
        use_output_layer = False
        self.left_decoder = JointTransformerDecoder(
            vocab_size,
            encoder_output_size,
            attention_heads,
            linear_units,
        ...
        self.right_decoder = JointTransformerDecoder(
            vocab_size,
            encoder_output_size,
            attention_heads,
            linear_units,
    ...
    def forward(
        ...
        Returns:
            (tuple): tuple containing:
                ...
                olens: (batch, )
        """
        #_x为decoder输出的特征向量，并非分类结果
        l_feature, _, olens = self.left_decoder(memory, memory_mask, ys_in_pad, dysf_in_pad,
                                          ys_in_lens)
        l_x = self.asr_output_layer(l_feature)
        r_x = torch.tensor(0.0)
        if reverse_weight > 0.0:
            r_feature, _, olens = self.right_decoder(memory, memory_mask, r_ys_in_pad, r_dysf_in_pad, 
                                               ys_in_lens)
            r_x = self.r_asr_output_layer(r_feature)
        return l_x, r_x, l_feature, r_feature, olens
