...
def count_P_R_F1(predict_dysf_number,
                  gold_dysf_number,
                  dysf_correct_number):
...
class Joint_Model(torch.nn.Module):
    ...
    def forward(
        ...
        if self.ctc_weight != 1.0:
            loss_att, acc_att, concat_out, concat_feature, dysf_out_pad = self._calc_att_loss(
        ...
        if self.dysf_weight != 0.0:
            dysf_loss, dysf_eval_info = self._calc_dysf_loss(concat_out, concat_feature, dysf_out_pad, split_info)
        else:
            dysf_loss = None
            dysf_eval_info = None
    ...
    def _calc_dysf_loss(self, concat_out, concat_feature, dysf_out_pad, split_info):
        ...
        return dysf_loss, dysf_eval_info

    def _accuracy_tagging(self, predict_result_tagging, gold_result_tagging, split_info) -> dict:
    ...
    def _calc_att_loss(
        ...
        return loss_att, acc_att * 100, concat_out, concat_feature, dysf_out_pad
