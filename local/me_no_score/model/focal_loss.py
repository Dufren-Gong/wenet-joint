...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        if self.reduction == 'mean':
            return focal_loss.sum() / torch.sum(targets != self.ignore_index)
...
class FocalLoss_before(nn.Module):
    def __init__(self, alpha=0.25, gamma=2, ignore_index=-1, reduction='mean'):
        """
        Focal Loss for multi-class classification.
