...
class Joint_Model(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.dysf_loss_type == "CE":
            ...
            self.dysf_loss_function = FocalLoss(alpha=[1, 3], gamma=2, ignore_index=ignore_id, reduction='mean')
    ...
    def _calc_dysf_loss(self, concat_out, concat_feature, dysf_out_pad, split_info):
        ...
        #同步ASR求loss
        dysf_loss = self.dysf_loss_function(dysf_predict.view(-1, dysf_predict.size(-1)), dysf_out_pad.view(-1))
        if self.dysf_loss_type == 'CE':
