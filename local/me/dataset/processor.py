...
def tokenize(sample, tokenizer: BaseTokenizer):
    ...
    dysf_labels, split_info = tokenizer.get_dysf_label(sample['txt'], sample['dysfl_label'])
    len_split = len(split_info)
    if 'score' in sample:
        score_temp = []
        score = [eval(i) for i in sample['score'].strip().split(' ')]
        this_score_index = 0
        for index in range(len_split - 1):
            score_temp.append(score[this_score_index])
            if split_info[index + 1] != -1:
                this_score_index += 1
        score_temp.append(score[this_score_index])
    else:
        score_temp = [1] * len_split
    assert len(tokens) == len(label) == len(dysf_labels) == len(score_temp)
    sample['tokens'] = tokens
    sample['label'] = label
    sample['dysfl_label'] = dysf_labels
    sample['split_info'] = split_info
    sample['score'] = score_temp
    return sample
...
def padding(data):
    ...
    sorted_dysf_labels = [
        torch.tensor(sample[i]['dysfl_label']) for i in order
    ]
    sorted_score = [
        torch.tensor(sample[i]['score']) for i in order
    ]
    sorted_split_info = [
        torch.tensor(sample[i]['split_info']) for i in order
    ]
    ...
    padding_labels = pad_sequence(sorted_labels,
                                  batch_first=True,
                                  padding_value=-1)
    padding_dysf_labels = pad_sequence(sorted_dysf_labels,
                                  batch_first=True,
                                  padding_value=-1)
    padding_score = pad_sequence(sorted_score,
                                  batch_first=True,
                                  padding_value=1)
    ...
    batch = {
        ...
        "target_dysf": padding_dysf_labels,
        "split_info": sorted_split_info,
