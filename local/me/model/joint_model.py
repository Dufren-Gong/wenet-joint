...
from wenet.utils.common import (IGNORE_ID, add_sos_eos, th_accuracy, count_P_R_F1, reverse_pad_list)
...
class Joint_Model(torch.nn.Module):
    ...
    def __init__(
        ...
        self.dysf_loss_type = dysf_loss_type
        if not asr_only:
            if self.dysf_loss_type == "CE":
                self.dysf_loss_function = torch.nn.CrossEntropyLoss(ignore_index=ignore_id, reduction='none')
            elif self.dysf_loss_type == 'Focal':
                self.dysf_loss_function = FocalLoss(alpha=focal_loss_alpha, gamma=2, ignore_index=ignore_id, reduction='none')
        else:
            assert dysf_head == None
    ...
    def forward(
        ...
        if self.dysf_weight != 0.0 and not self.asr_only:
            dysf_loss, dysf_eval_info, dysf_results = self._calc_dysf_loss(encoder_out, encoder_mask, concat_out, concat_feature, dysf_out_pad, split_info, score, device)
        else:
            dysf_loss = None
            dysf_eval_info = None
            dysf_results = None
        ...
        if temp_writer != None:
            for key_index, key in enumerate(keys):
                ...
                temp_dict = dict(key = key,
                                 ...
                                 dysf = dysf_results[key_index][:length].tolist() if dysf_results != None else target_dysf[key_index][:length].tolist(),
                                 split_info = split_info[key_index].tolist())
                write_line = json.dumps(temp_dict, ensure_ascii=False)
        ...
        info_dict = {
            ...
            "loss_att": loss_att,
            "loss_ctc": loss_ctc,
            "loss_dysf": dysf_loss if dysf_loss != None else torch.tensor(0),
            "th_accuracy": acc_att,
        }
    ...
    def _calc_dysf_loss(self, encoder_out, encoder_mask, concat_out, concat_feature, dysf_out_pad, split_info, score, device):
        #dysf_out_pad多了一个eos
        dysf_predict = self.dysf_head(encoder_out, encoder_mask, concat_out, concat_feature)
        #可以设置为1或者0，看看结果
        zero_column = torch.ones(score.shape[0], 1).to(device)
        add_eos_score = torch.cat((score, zero_column), dim=1)

        batch_size = concat_out.size(0)
        #同步ASR求loss
        if self.dysf_loss_type == 'CE':
            dysf_loss = self.dysf_loss_function(dysf_predict.view(-1, dysf_predict.size(-1)), dysf_out_pad.view(-1)) * add_eos_score.view(-1)
            dysf_loss = dysf_loss.sum() / batch_size
        elif self.dysf_loss_type == 'Focal':
            mask = dysf_out_pad != self.ignore_id
            add_eos_score = add_eos_score[mask]
            dysf_loss = self.dysf_loss_function(dysf_predict.view(-1, dysf_predict.size(-1)), dysf_out_pad.view(-1)) * add_eos_score.view(-1)
            dysf_loss = dysf_loss.sum() / batch_size
        dysf_eval_info = {}
        count_info = self._accuracy_tagging(dysf_predict, dysf_out_pad, split_info, split_only_count_first_flag=True)
        count_info_merge = self._accuracy_tagging(dysf_predict, dysf_out_pad, split_info, split_only_count_first_flag=False, use_score=False)
        count_info_merge_score = self._accuracy_tagging(dysf_predict, dysf_out_pad, split_info, split_only_count_first_flag=False, use_score=True)
        for key, value in count_info.items():
            dysf_eval_info[key] = value
            dysf_eval_info[f'merge_{key}'] = count_info_merge[key]
            dysf_eval_info[f'merge_score_{key}'] = count_info_merge_score[key]
        return dysf_loss, dysf_eval_info, self.get_predict_result(dysf_predict)

    def _accuracy_tagging(self, predict_result_tagging, gold_result_tagging, split_info, split_only_count_first_flag = True, use_score = True) -> dict:
        ...
        for j in range(0, len(gold_result_tagging)):
            ...
            predict_result_tmp = torch.softmax(predict_result_tmp)
            predict_tagging = torch.argmax(predict_result_tmp, dim=-1)
            predict_result = []
            start_token_index = [index for index, i in enumerate(split_info_tmp) if i != -1]
            start_token_index.append(len(split_info_tmp))
            #获取所有分词跨度
            gap = []
            for index in range(len(start_token_index) - 1):
                gap.append((start_token_index[index], start_token_index[index + 1] - 1))
            assert len(gap) == len(gold_result)
            for gap_index, this_gap in enumerate(gap):
                #如果无分词
                if this_gap[0] == this_gap[1]:
                    this_predict = predict_tagging[this_gap[0]]
                #如果有分词
                else:
                    split_num += 1
                    #如果直接赋值第一个标签
                    if split_only_count_first_flag:
                        ...
                        #获取分词的所有标签
                        this_all_tag = predict_tagging[this_gap[0]:this_gap[1] + 1]
                        zero_num = (this_all_tag == 0).sum().item()
                        one_num = (this_all_tag == 1).sum().item()
                        if zero_num > one_num:
                            this_predict = 0
                            ...
                            if use_score:
                                this_all_scores = predict_result_tmp[this_gap[0]:this_gap[1] + 1]
                                zero_scores = [this_all_scores[index][0] for index, i in enumerate(this_all_tag) if i == 0]
                                one_scores = [this_all_scores[index][1] for index, i in enumerate(this_all_tag) if i == 1]
                                zero_score = sum(zero_scores) / len(zero_scores)
                                one_score = sum(one_scores) / len(one_scores)
                                if zero_score > one_score:
                                    this_predict = 0
                                elif one_score > zero_score:
                                    this_predict = 1
                                else:
                                    this_predict = this_all_tag[0]
                            else:
                                this_predict = this_all_tag[0]
                    if this_predict == gold_result[gap_index]:
                        split_correct += 1
                predict_result.append(this_predict)
            assert len(predict_result) == len(gold_result)
            #gold中为disf的数量
