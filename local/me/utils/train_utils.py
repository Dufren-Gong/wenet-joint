from local.me.dataset.dataset import Dataset
import copy
...
from wenet.utils.common import StepTimer, tensor_to_scalar
...
def add_dataset_args(parser):
    ...
    parser.add_argument('--dysf_log_temp_dir',
                        required=True,
                        help='cv log dir')
    parser.add_argument('--symbol_table_path',
                        required=True,
                        help='symbol table dir')
...
def batch_forward(model, batch, scaler, info_dict, temp_writer = None, temp_compare = None):
    train_engine = info_dict.get('train_engine', "torch_ddp")
    ...
    if train_engine == "deepspeed":
        ...
        with torch.cuda.amp.autocast(enabled=dtype is not None,
                                     dtype=dtype,
                                     cache_enabled=False):
            loss_dict = model(batch, device, temp_writer, temp_compare)
    else:
        # torch_ddp
        # autocast context
...
def get_slim_num(num):
    if isinstance(num, (float, int)):
        value_str = str(num)
        if '.' in value_str:
            value_str = value_str.rstrip('0').rstrip('.')
            if isinstance(eval(value_str), float):
                value = round(num, 4)
            else:
                value = int(num)
        else:
...
def log_per_step(writer, info_dict, timer: Optional[StepTimer] = None):
    ...
    if (batch_idx + 1) % log_interval == 0:
        ...
        for name, value in loss_dict.items():
            if name != 'loss' and 'loss' in name and value is not None:
                log_str += '{} {} '.format(name, get_slim_num(value))
        if tag == "TRAIN":
...
def log_per_epoch(writer, info_dict):
    ...
    if int(os.environ.get('RANK', 0)) == 0:
        ...
        for name, value in loss_dict.items():
            writer.add_scalar('epoch/{}'.format(name), get_slim_num(value), epoch)
