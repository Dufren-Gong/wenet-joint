...
def compute_wer(ref_file, hyp_file):
    wer_info = {}
    ...
    with open(ref_file, 'r', encoding='utf-8') as ref_reader:
        lines = ref_reader.readlines()
    ref_reader.close()
    for line in lines:
        if tochar:
            array = characterize(line)
    ...
    if result['all'] != 0:
        ...
        wer = 0.0
    wer_info['Overall'] = dict(wer=wer, N=result['all'], C=result['cor'],
                                S=result['sub'], D=result['del'], I=result['ins'])
    ...
    if verbose:
        for cluster_id in default_clusters:
            ...
            if result['all'] != 0:
                ...
                wer = 0.0
            wer_info[cluster_id] = dict(wer=wer, N=result['all'], C=result['cor'],
                                S=result['sub'], D=result['del'], I=result['ins'])
