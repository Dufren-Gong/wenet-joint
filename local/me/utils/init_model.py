...
from local.me.model.joint_model import Joint_Model
from local.me.model.jointBiTransformerDecoder import JointBiTransformerDecoder
...
WENET_DECODER_CLASSES = {
    ...
    "joint_bitransformer": JointBiTransformerDecoder
}
...
WENET_MODEL_CLASSES = {
    ...
    'disf_asr_joint_model': Joint_Model
}


def init_model(args, configs):
    if args.asr_only:
        configs['model_conf']['asr_only'] = True
        configs['model_conf']['dysf_weight'] = 0
    ...
    elif model_type == 'disf_asr_joint_model':
        if args.asr_only:
            dysf_head = None
        else:
            dysf_head = DisfClassificationLayer(vocab_size,
                                            encoder.output_size(),
                                            **configs['dysf_classification_conf'])
        model = WENET_MODEL_CLASSES[model_type](
            vocab_size=vocab_size,
            encoder=encoder,
            decoder=decoder,
