...
def decode_text(lines: list, tokenizer):
    for line in lines:
        ...
        assert len(txt) == len(dysf) == len(split_info)
        txt_temp = []
        flag = 'stay'
        ...
        txt_temp = tokenizer.detokenize(txt_temp)[0].strip().split()
        txt_temp = ' '.join(txt_temp)
        #不删除dysf
        txt = tokenizer.detokenize(txt)[0].strip().split()
        txt = ' '.join(txt)
        line['text'] = txt
        line['del_dysf_text'] = txt_temp
    return lines
...
def trans_to_char(temp_path, tokenizer):
    ...
    writer_new.close()
    writer_new = check_remove_and_get_new(os.path.join(os.path.dirname(temp_path), info[0] + '_del_dysf.' + info[1]), 'w')
    for line in lines:
        write_line = ' '.join([line['key'], line['del_dysf_text']]) + '\n'
        writer_new.write(write_line)
    writer_new.close()
...
def main():
    ...
    # init tokenizer
    tokenizer = init_tokenizer(args, configs)
    ...
    if not args.asr_only:
        ...
        partens = ['acc', 'wer']
    dysf_temp_log_dir = args.dysf_log_temp_dir
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        if not args.asr_only:
            logging.info('            predict {} gold {} correct {} P {:.4f} R {:.4f} Aligned F1 {:.4f}'.format(
                loss_dict["predict"], loss_dict["gold"],loss_dict["correct"],
                loss_dict["p_score"], loss_dict["r_score"],loss_dict["f_score"]))
            logging.info('merge       predict {} gold {} correct {} P {:.4f} R {:.4f} Aligned F1 {:.4f}'.format(
                loss_dict["merge_predict"], loss_dict["merge_gold"],loss_dict["merge_correct"],
                loss_dict["merge_p_score"], loss_dict["merge_r_score"],loss_dict["merge_f_score"]))
            logging.info('merge score predict {} gold {} correct {} P {:.4f} R {:.4f} Aligned F1 {:.4f}'.format(
                loss_dict["merge_score_predict"], loss_dict["merge_score_gold"],loss_dict["merge_score_correct"],
                loss_dict["merge_score_p_score"], loss_dict["merge_score_r_score"],loss_dict["merge_score_f_score"]))
            logging.info('split info  only first label {:.4f}% merge label nums {:.4f}% merge label scores {:.4f}%'.format(
                loss_dict["split_correct"], loss_dict["merge_split_correct"],loss_dict["merge_score_split_correct"]))
        else:
            logging.info('asr only!')
        if first_epoch_flag:
        ...
        temp_compare_info = os.path.basename(temp_compare_path).strip().split('.')
        temp_writer_info = os.path.basename(temp_writer_path).strip().split('.')
        if not args.asr_only:
            dr_wer_info = compute_wer(os.path.join(os.path.dirname(temp_compare_path), temp_compare_info[0] + '_del_dysf.' + temp_compare_info[1]),
                                    os.path.join(os.path.dirname(temp_writer_path), temp_writer_info[0] + '_del_dysf.' + temp_writer_info[1]))
        else:
            dr_wer_info = compute_wer(os.path.join(os.path.dirname(temp_compare_path), temp_compare_info[0] + '_del_dysf.' + temp_compare_info[1]),
                                    temp_writer_path)
        ...
        logging.info('N {} C {} S {} D {} I {} WER {}'.format(
            wer_info["Overall"]['N'], wer_info["Overall"]['C'], wer_info["Overall"]['S'],
            wer_info["Overall"]['D'], wer_info["Overall"]['I'], wer_info["Overall"]['wer']))
        logging.info('N {} C {} S {} D {} I {} DR-WER {}'.format(
            dr_wer_info["Overall"]['N'], dr_wer_info["Overall"]['C'], dr_wer_info["Overall"]['S'],
            dr_wer_info["Overall"]['D'], dr_wer_info["Overall"]['I'], dr_wer_info["Overall"]['wer']))
