...
if [ $# != 1 -a $# != 2 ]; then
  echo "Usage: swbd1_data_prep.sh /path/to/SWBD set_mode"
  exit 1;
fi

SWBD_DIR=$1/wav
set_type=$2
swbd1_transcriptions_dir=$1/transcriptions/swb_ms98_transcriptions_split

dir=data/local/${set_type}
mkdir -p $dir
...
[ ! -f $swbd1_transcriptions_dir/sw-ms98-dict.text ] && \
...
}' $swbd1_transcriptions_dir/${set_type}/*/*/*-trans.text  > $dir/transcripts1.txt
...
# script]
mkdir -p data/${set_type}
for f in wav.scp text; do
  cp data/local/${set_type}/$f data/${set_type}/$f || exit 1;
done
...
utils/fix_data_dir.sh data/${set_type}
